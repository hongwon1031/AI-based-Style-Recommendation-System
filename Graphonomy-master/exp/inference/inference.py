...
def inference(net, img_path='', output_path='./', output_name='f', use_gpu=True):
    ...
    for iii, sample_batched in enumerate(zip(testloader_list, testloader_flip_list)):
        ...
        with torch.no_grad():
            # inputs를 GPU 또는 CPU로 이동
            inputs = inputs.to("cuda" if use_gpu else "cpu")

            # adj1_test, adj3_test, adj2_test를 GPU 또는 CPU로 이동
            adj1_test = adj1_test.to("cuda" if use_gpu else "cpu")
            adj3_test = adj3_test.to("cuda" if use_gpu else "cpu")
            adj2_test = adj2_test.to("cuda" if use_gpu else "cpu")
...
if __name__ == '__main__':
    ...
    if not opts.loadmodel == '':
        x = torch.load(opts.loadmodel, map_location=torch.device('cpu'))  # CPU로 로드
    ...
    if opts.use_gpu >0 :
        net.cpu()  # GPU 대신 CPU 사용
        use_gpu = False  # use_gpu를 False로 설정하여 CPU 사용
    else:
        use_gpu = False
        print("Running on CPU")

    inference(net=net, img_path=opts.img_path, output_path=opts.output_path, output_name=opts.output_name,use_gpu=False)
