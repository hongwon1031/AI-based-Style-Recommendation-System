...
class CPDatasetTest(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in self.c_names:
            ...
            cloth_mask_path = osp.join(self.data_path, 'cloth-mask', c_name[key])

            print(f"Loading cloth image from: {cloth_path}")
            print(f"Loading cloth mask from: {cloth_mask_path}")
            c[key] = Image.open(osp.join(self.data_path, 'cloth', c_name[key])).convert('RGB')
            c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', c_name[key]))
            cm[key] = transforms.Resize(self.fine_width, interpolation=0)(cm[key])
        ...
        person_image_path = osp.join(self.data_path, 'image', im_name)
        ...
        segmentation_path = osp.join(self.data_path, 'image-parse-v3', parse_name)
        print(f"Loading segmentation image from: {segmentation_path}")
        ...
        densepose_path = osp.join(self.data_path, densepose_name)
        print(f"Loading DensePose data from: {densepose_path}")
